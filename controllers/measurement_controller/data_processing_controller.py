        ...
        for key, value in self._data_processing.settings.legend.items():
            if key == NOTE_TO_TECH_KEY:
                ...
                self.view.widgets.sample_width_dsbox.setValue(
                    float(self._data_processing.settings.legend[THICKNESS_KEY]))
            elif key == MEASURE_OF_SAMPLE_KEY:
                self.view.widgets.sample_measurement_ledit.setText(
                    self._data_processing.settings.legend[MEASURE_OF_SAMPLE_KEY])
